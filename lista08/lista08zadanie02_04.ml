...
let (<.>) = zlozenie
(* 
	Podejmowałem próby, żeby zrobić to wg sygantury z poprzedniej części zadania.
	Jednak problem sprawiał fakt, że musimy przekazać do MyGraph dwa moduły,
	oraz dać swiatu zewnętrznemu dwa typy (vertex i edge). Jednak wg sygnatury
	z poprzedniego zadania, chcielibysmy wyswietlic typy, ktore zadeklarowalismy
	dopiero w srodku, i w momencie definicji modułu, są one nieznane.
 *)

module MyGraph (V : VERTEX) (E : (EDGE with type vertex = V.t)) : (GRAPH with type vertex = V.t and type edge = E.t) = struct
	(* module V = V2 *)
	type vertex = V.t
...
let tmp x = fun v -> MVertex.label v = x
